...
if __name__ == '__main__':
    ...
    for q in range(2):
        ...
        mask_out_dir = repo_dir + highlevel2 + "/uint8_binary_water_masks/"
        mask2_out_dir = repo_dir + highlevel1 + "/uint8_binary_water_masks/"
        ...
        for a in masklist:  # for each image listed
            ...
            # new_mask = stf.resize(orig_mask, (im_wid, im_hg), order=0)  # change res. of mask to correct CNN size
            pil_orig_mask = Image.open(mask_dir + a)
            new_mask = pil_orig_mask.resize((im_hg, im_wid), resample = Image.NEAREST)
            ...
            imgsave(mask_out_dir + x + '.png', new_mask)  # write the resized image
            imgsave(mask2_out_dir + x + '.png', orig_mask)  # write the original image
